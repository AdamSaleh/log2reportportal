...
.PHONY: install
## `install`: Fetch dependencies needed to run `log2reportportal`
install:
...
.PHONY: test-runner
test-runner:
	$(GOCMD) test ./... -race -covermode=atomic -coverprofile=./coverage/coverage.txt
	$(GOCMD) tool cover -html=./coverage/coverage.txt -o ./coverage/coverage.html
...
.PHONY: run
## :
## `run`: Run `log2reportportal` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `log2reportportal` in debug mode
...
.PHONY: docker-gen
## :
## `docker-gen`: Create a production docker image for `log2reportportal`
docker-gen:
...
.PHONY: docker-debug
## `docker-debug`: Create debug-friendly docker images for `log2reportportal`
docker-debug:
	echo "Building docker image \`$(IMAGE):$(VERSION)\`..."
