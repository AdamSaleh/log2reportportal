...
a docker containers for *log2reportportal*!
...
```bash
make docker-gen IMAGE=log2reportportal VERSION=latest
```

By default, the values for these environment variables are; <br>

```bash
IMAGE := log2reportportal
VERSION := latest
```
...
```bash
make clean-docker IMAGE=log2reportportal VERSION=latest
```


## Debug Images

The [dockerfile](./Dockerfile) for *log2reportportal* uses
...
```bash
make docker-debug IMAGE=log2reportportal-debug VERSION=latest
```
...
```bash
docker run log2reportportal-debug
```

You can run a debug image, and directly `shell` into it with

```bash
docker run --rm -it log2reportportal-debug bash
```
...
```bash
make clean-docker IMAGE=log2reportportal-debug VERSION=latest
```
