...
This section will guide you to setup a fully-functional local copy of `log2reportportal`
...
from [`log2reportportal's` Makefile][makefile-file] (most of which are
OS-independent)!
...
The pre-commit hooks used by `log2reportportal` are located within the
...
Tests in `log2reportportal` are classified as *fast* and *slow* - depending
...
|      `run`     	| Run *log2reportportal*                           	|       NA      	|
|  `docker-gen`  	| Create production docker image for *log2reportportal* 	|     docker    	|
|  `docker-debug`  	| Create debug-friendly docker image for *log2reportportal* 	|     docker    	|
...
```text
log2reportportal_<version>_<target-os>_<architecture>.<extension>
```
...
```sh
log2reportportal_v1.2.1_windows_x86_64.exe
```
...
To run `log2reportportal` in a docker container, read the instructions in
[docker section](./docker).

### Running `log2reportportal`

To run log2reportportal, use the command
